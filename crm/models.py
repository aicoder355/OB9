...
class Client(models.Model):
    ...
    name = models.CharField(max_length=100)
    # Accept formats: +992XXXXXXXXX, 992XXXXXXXXX, XXXXXXXXX (9 digits)
    phone_regex = RegexValidator(
        regex=r'^(?:\+?992\d{9}|\d{9})$',
        message="Номер телефона должен быть в формате +992XXXXXXXXX, 992XXXXXXXXX или XXXXXXXXX"
    )
    ...
    telegram_chat_id = models.BigIntegerField(null=True, blank=True, unique=True, help_text='Telegram chat id, привязанный к клиенту')
    telegram_username = models.CharField(max_length=100, blank=True, null=True, help_text='Telegram username клиента')
    registration_status = models.CharField(max_length=20, choices=REGISTRATION_STATUS_CHOICES, default='approved', help_text='Статус регистрации клиента')
...
class Driver(models.Model):
    ...
    # Telegram integration
    telegram_chat_id = models.BigIntegerField(null=True, blank=True, unique=True, help_text='Telegram chat id водителя')
    telegram_verified = models.BooleanField(default=False, help_text='Водитель верифицирован в Telegram')
    access_code = models.CharField(max_length=20, blank=True, null=True, help_text='Код доступа для авторизации в боте')
    access_code_created = models.DateTimeField(null=True, blank=True, help_text='Время создания кода доступа')
    ...
    def is_access_code_valid(self, code, expiry_hours=24):
        """Проверка валидности кода доступа"""
        if not self.access_code or not self.access_code_created:
            return False
        if self.access_code != code:
            return False
        # Проверка срока действия
        from datetime import timedelta
        expiry_time = self.access_code_created + timedelta(hours=expiry_hours)
