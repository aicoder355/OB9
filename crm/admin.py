...
class ClientAdmin(admin.ModelAdmin):
    list_display = ('name', 'client_type', 'phone', 'email', 'registration_status', 'telegram_chat_id', 'category', 'loyalty_points')
    list_filter = ('client_type', 'category', 'region', 'registration_status')
    search_fields = ('name', 'phone', 'email', 'company_name', 'tax_number', 'telegram_chat_id', 'telegram_username')
    fieldsets = (
        ('Основная информация', {
        ...
        ('Telegram интеграция', {
            'fields': ('telegram_chat_id', 'telegram_username', 'registration_status')
        }),
        ('Дополнительно', {
            'fields': ('notes', 'registration_date')
        })
    )
    readonly_fields = ('registration_date', 'loyalty_points', 'total_volume_purchased', 'last_purchase_date', 'telegram_chat_id', 'telegram_username')
...
class DriverAdmin(admin.ModelAdmin):
    list_display = ('name', 'phone', 'email', 'region', 'telegram_verified', 'user', 'active_orders_count')
    search_fields = ('name', 'phone', 'email', 'telegram_chat_id')
    list_filter = ('region', 'created_at', 'telegram_verified')
    fieldsets = (
        ('Основная информация', {
            'fields': ('name', 'phone', 'email', 'region', 'user')
        }),
        ('Telegram интеграция', {
            'fields': ('telegram_chat_id', 'telegram_verified', 'access_code', 'access_code_created')
        }),
        ('Системная информация', {
            'fields': ('created_at',)
        })
    )
    readonly_fields = ('created_at', 'telegram_chat_id', 'access_code_created')
